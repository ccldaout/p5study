...
def createLight(isize, rPower, gPower, bPower):
    ...
    for y in xrange(isize):
        for x in xrange(isize):
            factor = 10*exp(-((sq(center - x) + sq(center - y))/sq(isize/4)))
...
def actor(lightimg, center_x, center_y, radius, stepangles):
    ...
    while True:
        ...
        rotateY(yang)
        blendMode(LIGHTEST)
        image(lightimg, radius*cos(oang), radius*sin(oang))
        popMatrix()
        oang += ostep
        xang += xstep
        yang += ystep
...
def setup():
    ...
    scene_controller.add_generator(actor(createLight(50, 0.1, 0.5, 0.4),
                                         200, 200, 150,
                                         (HALF_PI/30, 0.001, 0.005)))
    scene_controller.add_generator(actor(createLight(40, 0.9, 0.3, 0.4),
                                         200, 200, 130,
                                         (HALF_PI/20, -0.001, 0.002)))
